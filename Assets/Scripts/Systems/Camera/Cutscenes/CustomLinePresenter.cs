/* if i have time ill get this working but for now leave this.
...
// base taken from LinePresenter.cs (given from yarn)
public class CustomLinePresenter : DialoguePresenterBase
{
    ...
    public override async YarnTask RunLineAsync(LocalizedLine line, LineCancellationToken token)
    {
        ...
        // handle metadata b4 showing text
        if (line.Metadata != null && line.Metadata.Length > 0)
        {
            foreach (var tag in line.Metadata)
            {
                await HandleCutsceneMetadata(tag);
            }
        }
        ...
        // at this point, all text is shown
    ...
    private async YarnTask HandleCutsceneMetadata(string tag)
    {
        // Split the tag string by spaces or '#' markers
        // e.g. "cutsceneAction:UnitMoveToPos #unit:Ylru #x:3 #y:2"
        ...
        if (!args.TryGetValue("cutsceneAction", out string action))
            return; // not a cutscene action tag

        // dispatch based on action name
        switch (action)
        {
            case "UnitMoveToPos":
                if (args.TryGetValue("unit", out var unitName) &&
                    args.TryGetValue("x", out var sx) &&
                    args.TryGetValue("y", out var sy) &&
                    float.TryParse(sx, out float x) &&
                    float.TryParse(sy, out float y))
                {
                    await CutsceneManager.Instance.YarnCoroutine(
            ...
            case "UnitJump":
                if (args.TryGetValue("unit", out var u) &&
                    args.TryGetValue("power", out var sp) &&
                    float.TryParse(sp, out float power))
                {
                    await CutsceneManager.Instance.YarnCoroutine(
                        UnitManager.Instance.JumpUnit(u, power)
                    );
                }
                break;

            default:
                Debug.LogWarning($"Unknown cutsceneAction '{action}'");
                break;
        }
    }
}
